<div>
    <x-nawasara-ui::page-header
        title="Nawasara News"
        description="Mirror artikel berita dari website ponorogo.go.id (read-only — sumber data ada di ponorogo.go.id)."
        :count="$totalArticles . ' artikel, ' . $categories->count() . ' kategori'">
        ...
        <div class="flex flex-col md:flex-row md:flex-nowrap md:items-center gap-2">
            <div class="flex flex-wrap items-center gap-2 shrink-0">
                {{-- <x-nawasara-ui::filter-panel
                    ...
                    <x-nawasara-ui::filter-group label="Kategori" model="categoryId" :items="$categoryOptions" icon="lucide-circle-check" />
                </x-nawasara-ui::filter-panel> --}}
            </div>
    ...
    <x-nawasara-ui::modal id="news-detail" maxWidth="2xl" :title="$this->detail?->judul ?? 'Detail Artikel'">
        @if ($this->detail)
            ...
            <div class="grid grid-cols-2 gap-4 text-sm mb-4">
                <div><span class="text-gray-500 dark:text-neutral-400">Kategori:</span> <span
                        class="font-medium text-gray-800 dark:text-neutral-200">{{ $d->category->slug ?? '—' }}</span>
                </div>
